...
交接至少包含运行清单、`.m` 源码、配置、`ver`/许可记录、输入清单、日志、baseline/主模型指标、结果表、图、`CONTRIB-*` 专门证明产物、失败与降级说明、批处理复现命令和输出哈希。无专门证据的亮点必须保持 `CANDIDATE` 或降级为 `DROPPED`。
