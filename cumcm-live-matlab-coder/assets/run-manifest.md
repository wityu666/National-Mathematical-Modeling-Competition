...
扇形图、水平条形图、折线图、竖向柱形图等均为候选图型，不要求全部出现；只登记实际使用且能回答分析任务的图。未使用的类型可记为 `NOT_APPLICABLE`，不得为图型覆盖率或正文页数补造装饰图。

| `figure_id` | 小问 | 唯一结论 | 分析任务 | 图型及选择理由 | palette_set | 图源数据 | 生成函数/命令 | PDF/PNG | 最终尺寸/灰度 QA |
|---|---|---|---|---|---|---|---|---|---|
