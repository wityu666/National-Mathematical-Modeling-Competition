...
上表是候选图型库，不是覆盖清单。扇形图、水平条形图、折线图、竖向柱形图允许有几种或全部不出现；只有能直接回答题面、模型或验证任务时才生成。3D 曲面只在 z 维具有真实空间/机理含义且二维等值图不能表达时使用，不得为了图型丰富度或正文页数自动补图。
...
- 柱图默认从零起；
- 需要放大非零差异时改用点图/区间图；
- 扇形图/饼图只在不超过 5–6 个类别、数据非负、总量有明确意义且“部分—整体”是核心问题时使用，直接标注类别与占比，禁止 3D/爆炸饼；
- 占比接近、类别更多或需要精确比较时，改用排序水平条形图或 100% 堆积条；
- 水平条形图用于排序、长标签或较多类别，并按数值或题面逻辑排列；竖向柱形图用于少量离散类别且数值轴原则上从零起；
- 折线图只连接时间、迭代、参数或其他真实有序横轴，不得连接无序类别制造趋势；
- 双纵轴只有在量纲不同且关系确实必须共图时使用，并明确两侧颜色和尺度；
- 不用面积或体积编码精确数值。

可按冻结配置调用 `pie`、`barh`、`plot` 和 `bar`；所用颜色必须来自同一个 `style` 和冻结的 `object_color_map`。这些基础图型均为可选项，未使用时标记为 `NOT_APPLICABLE`，不得生成空图、重复图或为覆盖图型而造图。
