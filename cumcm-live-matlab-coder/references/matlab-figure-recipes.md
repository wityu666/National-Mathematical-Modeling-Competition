...
`cfg.palette_set` 必须由团队在开始出图前显式选为 `SET-A`、`SET-B`、`SET-C` 或 `SET-D`，并记录到运行清单。全文只使用该组且保持对象映射一致，不提供自动默认，也不得逐图换组。四组均按 CIELAB L* 单调阶梯设计，相邻系列实测 ΔL* 不低于 12；具体色值和色相性格见写作 Skill 的图表手册。不得退回 MATLAB 默认高饱和 `ColorOrder`；颜色之外仍同时使用线型、标记、纹理或直接标签作为冗余编码。
