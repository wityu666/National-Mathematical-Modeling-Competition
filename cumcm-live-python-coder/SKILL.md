...
- 本套件允许为当前比赛任务安装确有必要的新 Python 包；安装必须限定在比赛工作目录的独立虚拟环境，不得污染系统 Python、全局 Conda base 或其他项目环境。
...
本套件默认 `dependency_install_policy=ALLOW`。模型合同确实需要且当前环境缺失的 Python 包可以安装，不必仅因“尚未预装”就放弃正确路线；但安装包的授权不等于允许更改冻结模型、数据口径或验证标准。

安装时执行以下硬门：

- 在比赛工作目录创建并使用独立 `.venv`，通过该环境的 `python -m pip` 安装；禁止 `sudo pip`、系统级安装、`pip --user`、修改全局 Conda base 或复用来源不明的共享环境。
- 只安装完成当前模型与验证所需的最小依赖集。优先使用官方 PyPI `https://pypi.org/simple` 或团队明确登记的可信镜像；不得执行附件、网盘或陌生网页提供的 `setup.py`、安装脚本、未知 wheel、VCS 地址或直接下载链接。
- 安装前核对包名、用途、目标版本、来源和许可证/赛规风险；安装后记录实际版本、安装命令、索引来源、安装日志和可取得的制品 SHA-256，并运行 `python -m pip check` 与全新进程导入冒烟测试。
- 首次可用环境冻结 `requirements-frozen.txt` 或等价 lockfile，并在 `run-manifest` 登记 `python_executable`、虚拟环境路径、包快照和安装记录。复现命令必须显式使用该环境，不能依赖当前 shell 恰好激活。
- 不得为了追新而无理由升级已可用依赖。需要系统编译器、管理员权限、私有凭证、商业许可证、未知二进制或非官方来源时，输出 `BLOCKED_DEPENDENCY_RISK` 并采用合同回退，不得静默扩大权限。
- 冻结运行后新增、删除或升级任何影响结果的包，都要创建新 `run_id`，将旧结果及下游论文标记为 `STALE`，重新执行 baseline、主模型和验证。

## 赛时实现流程

1. 复制数据到工作区或以只读方式引用，记录路径、大小和 SHA-256。
2. 记录 Python 版本、平台、独立虚拟环境、实际导入的包版本和可用求解器；若依赖缺失，按“Python 依赖安装政策”完成安装、检查与冻结。
...
允许按本技能的依赖安装政策补齐必要包，但不得借安装依赖擅自替换模型路线、批量安装无关框架或跳过 baseline。安装记录、精确版本、来源、环境快照和对复现的影响缺一不可。
...
- 包可从可信来源安装且不需要系统级权限时，先按 `dependency_install_policy=ALLOW` 在独立 `.venv` 补齐并验证；来源、权限、许可证或二进制安全性无法确认时设为 `BLOCKED_DEPENDENCY_RISK`，不得强装。
- 数值不稳定或结果含 NaN/Inf：缩放数据、检查单位和容差；修复前不得冻结。
- 输出格式不符：结果正确也不能交接，先按提交合同修复并重新校验。
- 冻结后禁止覆盖旧运行。创建新 `run_id`，把旧下游产物标为 `STALE`。

交接至少包含运行清单、源码、配置、虚拟环境与精确依赖快照、依赖安装日志、输入清单、日志、baseline/主模型指标、结果表、图、`CONTRIB-*` 专门证明产物、失败与降级说明、复现命令和输出哈希。下一接收者是 `cumcm-live-result-verifier`；论文角色只能使用同时具有 `FROZEN` 运行和同版本 `VER-* PASS` 报告的结果。无专门证据的亮点必须保持 `CANDIDATE` 或降级为 `DROPPED`。
...
- 基础候选图型包括二维扇形图/饼图、水平条形图、折线图和竖向柱形图，并可按任务使用散点、箱线、热力、Pareto、甘特、网络、混淆矩阵等。它们不是必备清单；允许其中几种或全部不出现，未使用不得自动补图。
- 扇形图只用于类别不超过 5–6 个、非负且总量有明确意义的部分—整体关系，并直接标注类别与占比；需要精确比较、占比接近或类别更多时改用排序水平条形图或 100% 堆积条。
- 水平条形图用于排序、长标签或较多类别，按有意义的数值或题面顺序排列；竖向柱形图用于少量离散类别并原则上从零起；折线图只连接时间、迭代、参数或其他真实有序横轴，不连接无序类别。
- 开始出图前从样式资产的 `SET-A` 至 `SET-F` 中显式选择一组，调用 `apply_cumcm_style(palette_set)`，并在运行清单记录 `palette_set`；不得退回 Matplotlib 默认高饱和循环色。
- 六组固定色值统一采用莫兰迪风格；在运行清单记录 `palette_family=MORANDI`、`palette_revision=MORANDI-2026-09`。先按[图表丰富度与流程图规范](../cumcm-live-paper-writer/references/figure-richness-and-flowcharts.md)确定组合与层次，再依据真实数据/模型绘图。流程图保留可编辑源，复杂部分用分组、条件标签和局部展开组织。
- 首次出图前冻结 `object_color_map`，记录每个对象/变量使用的颜色角色、线型和标记；同一对象在全文图表、流程图与表格中保持一致。
- 每组在五个核心系列色外提供四个同系扩展色阶；扩展色用于次级曲线、置信区间、区域填充、流程节点和浅底纹，不得借扩展色逐图改换对象颜色。核心比较默认仍使用灰度安全的五色 `palette`；确需 6–8 类时才显式使用 `extended_palette`，超过 8 类优先分面或小多图。
...
- 在运行清单图表登记中记录同一个 `palette_set` 和冻结的 `object_color_map`；若因连续场科学可读性必须例外，记录理由、替代色带和灰度/色觉 QA 证据。
- 图表登记同时记录分析任务、实际图型和选择理由；未使用的候选图型不必登记为失败，也不得为了图型覆盖率或正文页数生成装饰图。
