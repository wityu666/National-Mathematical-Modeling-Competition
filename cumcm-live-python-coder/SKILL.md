...
- 默认调用 `apply_cumcm_style()` 使用固定的 `cumcm-morandi-v1` 低饱和莫兰迪色板；不得退回 Matplotlib 默认高饱和循环色。
- 同一对象跨图保持颜色、线型和标记一致。
- 莫兰迪颜色不能作为唯一编码；比较组必须同时使用线型、标记、填充纹理或直接标签，重点对象使用色板内的灰豆沙红，不临时加入荧光色。
...
- 在运行清单图表登记中记录 `palette_id=cumcm-morandi-v1`；若因连续场科学可读性必须例外，记录理由、替代色带和灰度/色觉 QA 证据。
