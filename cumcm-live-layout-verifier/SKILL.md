---
name: cumcm-live-layout-verifier
description: 比赛进行中用于中国大学生数学建模竞赛 A、B、C 题论文初次成稿后的重复排版复核。适用于检查 Word/LaTeX 源、编译日志和真实渲染 PDF 的字体、裁切、重叠、分页、图表公式可读性、匿名信息、批注修订与占位符，并驱动“发现—修复—重新生成—重查”闭环；应在 paper-writer 之后、final-auditor 之前使用。
---

# CUMCM 论文排版复核

## 定位

本技能只验证论文的版式与渲染质量，不重新判断数学结论。先由 `cumcm-live-paper-writer` 生成提交候选 PDF，再用本技能重复检查；只有当前 PDF 的 `LAYOUT-*` 报告为 `PASS`，才能交给 `cumcm-live-final-auditor`。

静态脚本通过仅表示 `PRECHECK_PASS`，不能代替真实 PDF 逐页查看。不得把“编译成功”“可以打开”或文本抽取正常当成排版通过。

开始前读取：

- 完整复核协议：[references/layout-verification-protocol.md](references/layout-verification-protocol.md)
- 报告模板：[assets/layout-report.md](assets/layout-report.md)

## 1. 冻结复核对象

记录：

1. `layout_id`，格式 `LAYOUT-###`；
2. 主排版路线、源文件路径与源版本；
3. 最终 PDF 路径、SHA-256、生成命令和生成时间；
4. 当届官方版式、匿名、首页、页数和文件大小规则的证据位置；
5. 上游论文版本、`VER-* PASS` 与 `CONTRIB-*` 账本版本。

任一源文件、图表、字体、编译配置或 PDF 字节变化，旧 `LAYOUT-* PASS` 立即变为 `STALE`。

## 2. Round A：自动预检

执行：

```bash
python3 scripts/layout_preflight.py /absolute/path/to/final.pdf \
  --source /absolute/path/to/paper-source \
  --json
```

只有当届规则明确给出 PDF 大小限制时再加入：

```bash
--max-pdf-mb 20
```

脚本只读检查 PDF 文件头/尾、文件大小、源文件占位符、LaTeX 致命错误与未定义引用、明显 overfull 风险、Word 批注/修订、宏文档，并在系统工具可用时检查 PDF 元数据、字体和文本提取。退出码：

- `0`：`PRECHECK_PASS`，仍必须完成 Round B；
- `1`：发现阻塞项；
- `2`：参数或路径错误。

找不到外部 PDF 工具时记录能力缺口，不得据此直接判定阻塞或假装已检查。

## 3. Round B：真实渲染复核

将当前 PDF 每页渲染为图片并查看。页数较少时检查全部页面；时间极紧也至少覆盖：

- 首页、摘要、目录（如有）、每章起始页、参考文献、附录和最后一页；
- 每一页跨页表、长公式、密集多子图、流程图、地图、代码或大段引用；
- 静态预检指出风险的页面及其前后页。

逐页检查：

- 文字、公式、图表无裁切、重叠、乱码、缺字、字体替换和异常空白；
- 页码、页眉页脚、章节层级、段前段后、行距与分页一致；
- 正文“符号说明”表位于模型假设之后、数据/建模章节之前，没有被移入附录；
- 符号表列宽合理，数学符号、上下标、单位和取值范围清晰；跨页时重复表头，不出现表头与内容分离、整行截断或字号过小；
- 不出现孤立标题、标题落在页末、正文孤行寡行或不合理整页留白；
- 图例、坐标、单位、题注、表头、续表和公式编号完整、清楚、对齐；
- 最终插入尺寸下图中文字可读，灰度打印和色觉缺陷下仍能区分；
- 首页、匿名信息、学校/队员信息、文件元数据符合当届规则；
- Word 无批注、修订、隐藏文本和模板提示；LaTeX 无未解决引用或警告造成的可见缺陷。

每个问题使用 `LAYOUT-ISSUE-###`，记录页码、对象、截图、级别、修复动作和复查结果。

## 4. 重复修复闭环

默认最多进行三轮：

```text
Round A 预检 -> Round B 逐页查看 -> 登记问题
-> paper-writer 修复源文件 -> 完整重新生成 PDF
-> 更新 SHA-256 -> 原报告 STALE -> 从 Round A 重新检查
```

严重级别：

- `P0`：违规/身份泄露、内容丢失、公式或关键表图不可读、提交格式不符；
- `P1`：明显影响评阅，如重叠、错误分页、小字、断裂公式、图表标签缺失；
- `P2`：不阻塞提交但值得修复的轻微间距、对齐或视觉一致性问题。

同一错误连续两轮未修复时停止局部微调，回退到最近可验证版本，检查模板、字体、导出链或源对象。临近截止不得为了轻微美化破坏已验证的可提交版本。

## 5. 判定

只有同时满足以下条件才能写 `PASS`：

- 当前 PDF SHA-256 与报告完全一致；
- Round A 为 `PRECHECK_PASS`；
- Round B 已检查规定范围并保存页码/截图证据；
- 当届排版与匿名硬规则已核验；
- 正文符号说明表的位置、列宽、跨页和可读性已检查；
- 所有 `P0/P1` 已关闭；
- 修复后的 PDF 已从 Round A 重新复核，而非只看局部截图；
- 报告状态为 `FROZEN`。

未取得官方规则、无法渲染 PDF、未完成视觉检查或存在开放 `P0/P1` 时，输出 `BLOCKED`，不得使用“基本通过”。

## 6. 交接

把 `layout-report.md`、最终 PDF、源版本和截图目录交给 `cumcm-live-final-auditor`。终审仍要独立抽查排版，不能直接继承本技能结论。

## 完成条件

- 已生成一份绑定当前 PDF SHA-256 的 `LAYOUT-*` 报告；
- 自动预检、逐页视觉复核和至少一次修复后重查（若发现问题）均有记录；
- `PASS` 只属于当前冻结 PDF，文件变化会自动失效；
- 最终审计输入中已包含本报告。
