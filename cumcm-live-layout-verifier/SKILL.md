...
description: 比赛进行中用于中国大学生数学建模竞赛 A、B、C 题论文初次成稿后的重复排版复核。适用于检查同一冻结内容的可编辑 Word（.docx）与 PDF 两版、Word/LaTeX 主排版源、编译导出日志以及真实渲染效果，强制核验编号正文默认 26–30 页（可配置）、摘要与目录不计页数、附录不限页和附录主要建模代码，并核对双版本一致性、字体、裁切、重叠、分页、图表公式可读性、匿名信息、批注修订与占位符；应在 paper-writer 之后、final-auditor 之前使用。
---
...
8. 附录代码主题 `code_theme`、实际等宽字体与官方黑白例外证据；默认彩色主题为 `VS-CODE-LIGHT-PLUS`，且与图表 `palette_set` 分开登记。
...
- 附录关键建模代码使用可复制的等宽文本而非截图，字号、缩进、换行、跨页和行号/代码标识清晰；长行不越过页边界，分页不截断到无法理解。全部代码使用同一 `code_theme`：当届允许彩色时采用 `VS-CODE-LIGHT-PLUS`，按语义角色使用Light+ 语义多色字体、极浅灰底，关键字加粗、注释斜体；不得逐块换色、随机配色、使用深色编辑器背景，也不得与图表 `palette_set` 混用；
- 对附录代码同时执行彩色、灰度与复制抽查：Word/PDF 的 token 文字、字体颜色、加粗/斜体、缩进和换行一致，灰度下仍可依靠字形与语法理解，复制到纯文本后与冻结源代码一致。官方要求黑白时只接受已登记的 `MONOCHROME_OFFICIAL_OVERRIDE` 和规则证据；
- `--appendix-code-page` 指向的页面确实开始出现最终采用模型的主要代码，而不是导包、通用绘图、重复文件读写或未采用算法；只登记页码但页面没有主要代码时仍输出 `BLOCKED_APPENDIX_CODE`；
- 提交论文必须包含附录 A 文件名称表与附录 B 问题求解代码。附录 A 的“文件名称、文件说明”两列须与实际交付文件一致，采用只有顶线、表头下横线和底线的三线表，无竖线或逐行横线。附录 B 按 Q1、Q2、Q3 排列，有 Q4 时增加 Q4、无 Q4 时删除该节；每节注明与附录 A 一致的文件名称。结构、文件表或三线表不符合时记为 `P1` 并修复；任一实际小问的主要代码缺失仍记为 `BLOCKED_APPENDIX_CODE/P0`。内部冻结清单/哈希、验证报告、事实追踪表、图形注册表和 AI 交互文件索引混入时记为 `P1` 并退回删除，但保留有效的附录 A 文件名称表；本机绝对路径、身份或密钥等敏感信息泄露按 `P0` 处理。
...
- `P1`：明显影响评阅，如重叠、错误分页、小字、断裂公式、图表标签缺失，或在无官方黑白例外时未使用统一多色代码主题、主题跨代码块不一致、Word/PDF 高亮映射不一致；
...
- 附录代码 `code_theme` 已登记；语义多色、关键字加粗、注释斜体、极浅灰底、Word/PDF 一致性、灰度可读性和纯文本复制均已检查，或官方黑白例外证据完整；
- 已核对附录 A 文件名称三线表和附录 B 各小问代码，记录 `appendix_file_table_pdf_page` 与 `appendix_question_pages`，无缺失问题或多余的 Q4；内部审计清单继续在论文外部留存。
