...
8. 附录代码主题 `code_theme`、实际等宽字体与官方黑白例外证据；默认彩色主题为 `VS-CODE-LIGHT-MUTED`，且与图表 `palette_set` 分开登记。
...
- 附录关键建模代码使用可复制的等宽文本而非截图，字号、缩进、换行、跨页和行号/代码标识清晰；长行不越过页边界，分页不截断到无法理解。全部代码使用同一 `code_theme`：当届允许彩色时采用 `VS-CODE-LIGHT-MUTED`，按语义角色使用克制的多色字体、白底，关键字加粗、注释斜体；不得逐块换色、随机配色、使用深色编辑器背景，也不得与图表 `palette_set` 混用；
...
- 附录代码 `code_theme` 已登记；语义多色、关键字加粗、注释斜体、白底、Word/PDF 一致性、灰度可读性和纯文本复制均已检查，或官方黑白例外证据完整；
