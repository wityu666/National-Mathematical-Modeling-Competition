...
- 上述 `CODE-*` 映射、完整工程路径、SHA-256、冻结清单、验证报告、事实追踪表和图形注册表只在论文外部的交付/审计材料中登记；提交论文不得设置“完整程序与支撑材料索引”“支撑材料清单”或类似章节，也不得逐项展示内部工程文件列表；
- 新进程重跑证据可用；
...
- 论文附录不存在完整程序与支撑材料索引、支撑材料清单或内部工程文件列表；若命中则按 `P1` 删除，涉及绝对路径、身份、密钥等敏感信息时升级为 `P0`；
- 目录清单与 SHA-256 已冻结。
