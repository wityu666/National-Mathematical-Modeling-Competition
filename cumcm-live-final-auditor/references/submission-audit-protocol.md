...
本协议是 Skill 自带的原创审计规则，不依赖作者的本机资料或历届模板。年度页数、大小、匿名或 AI 要求必须由当届官方规则证明；编号正文 26–30 页、摘要不计、附录不限页是用户已确认的内部质量门，不得冒充当届官方要求。
...
- 摘要起止页、编号正文第一章起始页、附录起始页、总页数、编号正文实测页数和边界截图；
- 审计工具、版本和命令。
...
- `P0`：无法提交、违规、正文低于有效下限或超过有效上限、正文注水、结论错误、身份泄露、危险文件、关键数字不可追溯或论文亮点无冻结证据；
...
对“问题分析”末尾的 `FIG-OVERVIEW-001` 从图到合同、再从合同回到图双向核对：

1. 每个实际小问均覆盖输入/数据或上问产物、关键处理/假设、模型或数学构造、算法/求解器、真实检验和输出/答案；
2. 共享模块、传递变量/文件、依赖方向或“相互独立”标识与 `problem_contract/model_contract/run_manifest/verification-report` 一致；
3. 图中没有正文未采用的方法、未执行的验证、未交付的输出和虚构的跨问关系；
4. `FIG-OVERVIEW-001` 能定位到可编辑源、生成入口、图源合同、冻结版本和 Word/PDF 两版位置；
5. 最终尺寸下节点、箭头、题号边界和直接标签可读，使用全文冻结的 `palette_set/object_color_map`，颜色不是唯一编码；
6. 没有照抄参考论文或示例图片的成套节点文字、固定布局、色块顺序和连接方式，也没有水印、广告、机构标识或截图界面。

缺图或漏掉任一小问为 `P1`；与核心模型矛盾、写入未采用方法/未执行验证、虚构依赖为 `P0`。仅有“数据→模型→结果”等通用空框、重复正文的装饰流程图或异常放大以凑页数，按反注水门记为 `P0`。方框与箭头属于通用图形语法，本身不计模板指纹。
...
- 公式、变量、参数、目标和约束与代码一致；
- 正文符号说明位于模型假设之后、数据/建模章节之前，并采用只含顶线、表头下横线和底线的三线表；不得使用竖线、左右边线、逐行横线或全边框网格。跨页时重复表头并保持续表线型与列对齐；
- baseline 与主模型同口径比较；
- 预测有样本外验证；
- 分类有类别指标和阈值；
- 评价有权重与排名敏感性；
...
- 附录收录实际生成最终模型、求解结果或关键验证的关键建模代码，并通过 `CODE-*`、相对源文件、函数/脚本段、冻结版本和 `MODEL/RUN/RID/FIG/TAB` 映射到完整源代码与正文结果；
- 附录代码以可复制文本呈现，和冻结源代码逐项一致；完整工程的相对路径与 SHA-256 可定位；
- 当届允许彩色时，附录全部代码统一使用 `code_theme=VS-CODE-LIGHT-PLUS`，极浅灰底等宽并按语义角色使用多色字体，关键字加粗、注释斜体；Word/PDF 的主题、token 映射、灰度可读性和纯文本复制抽查一致。代码主题与图表 `palette_set` 分开登记，不逐块换色、不随机配色、不用深色编辑器背景；官方黑白例外须有 `MONOCHROME_OFFICIAL_OVERRIDE` 和规则证据；
- 论文使用附录 A 文件名称三线表和附录 B 问题求解代码。附录 A 列“文件名称、文件说明”，与实际交付文件一一对应；附录 B 按 Q1、Q2、Q3 排列，有 Q4 时增加 Q4，无 Q4 时删除该节。每节代码对应附录 A 文件名称；`CODE-*` 映射、SHA-256、冻结清单、验证报告、事实追踪表和图形注册表仍在论文外部的交付/审计材料中登记；
- 新进程重跑证据可用；
...
- 编号正文实测处于有效区间（默认 26–30 页），摘要未计入，附录页数不限，摘要/编号正文/附录物理页边界有截图；当届官方上限低于 26 页时已记录下限失效证据；
- 正文没有通过放大图文、扩大字号行距或页边距、搬入完整代码/原始数据、重复内容、装饰图表或未使用算法与推导来凑足下限；
- 附录关键建模代码存在、可读、可复制，并与提交包中的冻结源代码一致；
- 附录 B 各实际小问均有主要代码，Q4 的存在性与题目一致；记录 `appendix_file_table_pdf_page` 与 `appendix_question_pages`，`appendix_start_pdf_page` 指向 A，`appendix_code_pdf_page` 指向 B 首个实际代码页。缺少任一实际小问的主要代码为 `BLOCKED_APPENDIX_CODE/P0`；
- 附录代码主题统一且 Word/PDF 多色语义高亮一致，灰度仍可读，复制抽查不改变字符、缩进或换行；缺少主题或映射不一致为 `P1`，截图、不可复制或代码文本变化为 `P0`；
- 附录 A 的文件名称表采用三线表，只有顶线、表头下横线和底线，无竖线或逐行横线；Word/PDF 内容一致。A/B 结构、文件对应或表格格式错误按 `P1` 修复，内部审计字段混入按 `P1` 删除，绝对路径、身份、密钥等敏感信息按 `P0` 处理；不得误删有效的文件名称表；
- 目录清单与 SHA-256 已冻结。
