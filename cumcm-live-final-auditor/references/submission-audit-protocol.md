...
本协议是 Skill 自带的原创审计规则，不依赖作者的本机资料或历届模板。年度页数、大小、匿名或 AI 要求必须由当届官方规则证明；正文 24–30 页是用户已确认的内部质量门，不得冒充当届官方要求。
...
24 <= main_body_pages <= 30
```

没有附录时正文计到 PDF 最后一页。附录页数不设上限。24 页下限是用户已确认的内部质量门，不是官方要求；若当届官方上限低于 24 页，下限自动失效并以官方上限为准。正文/附录边界必须由最终 PDF 物理页和章节内容共同证明；核心模型、结果、验证、符号表或复现入口不得为规避上限被移入附录。
...
- 正文实测处于有效区间（默认 24–30 页），附录页数不限，正文/附录物理页边界有截图；当届官方上限低于 24 页时已记录下限失效证据；
