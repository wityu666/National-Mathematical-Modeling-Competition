...
- 当届允许彩色时，附录全部代码统一使用 `code_theme=VS-CODE-LIGHT-MUTED`，白底等宽并按语义角色使用多色字体，关键字加粗、注释斜体；Word/PDF 的主题、token 映射、灰度可读性和纯文本复制抽查一致。代码主题与图表 `palette_set` 分开登记，不逐块换色、不随机配色、不用深色编辑器背景；官方黑白例外须有 `MONOCHROME_OFFICIAL_OVERRIDE` 和规则证据；
