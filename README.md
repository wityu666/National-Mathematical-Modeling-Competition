...
- 编号正文实测页数须满足用户已确认的内部质量门 26–30 页：从第一章“问题重述”所在 PDF 物理页计至附录前一页，摘要、关键词和目录等正文前置部分不计入，附录页数不设上限。必须记录摘要结束页、编号正文起始页、附录起始页和实测正文页数；缺少任一页数边界立即 `BLOCKED_PAGE_BOUNDARY/P0`，低于下限或超过上限立即 `BLOCKED_PAGE_RANGE/P0`，`layout_preflight.py` 不再允许 `UNVERIFIED` 以退出码 0 通过。若当届官方上限低于 26 页，26 页下限自动失效并以官方上限为准；不得靠放大图文、扩大间距、重复内容或无关材料注水凑页数。
...
- 论文图表以莫兰迪色系为主，登记 `palette_family=MORANDI`、`palette_revision=MORANDI-2026-09`，仍显式选择一个 `palette_set` 并冻结 `object_color_map`。按总览、数据、模型、算法、结果与验证规划互补图表；较复杂流程图可采用分组、泳道、条件分支、反馈和局部展开，结构来自真实模型与求解过程。
- 逐篇国奖论文的图号、页码、视觉特点和可迁移方法记录在[图表学习记录](cumcm-live-paper-writer/references/award-figure-studies.md)，图表组合与流程图细则见[设计规范](cumcm-live-paper-writer/references/figure-richness-and-flowcharts.md)。未读取的论文明确标记，不作为已学结论。
- 正文“符号说明”必须使用三线表，仅保留顶线、表头下横线和底线，不使用竖线、左右边线、逐行横线或全边框网格；跨页时重复表头，并核对 Word/PDF 两版线型一致。
- 提交论文附录按“附录 A 文件名称表 → 附录 B 问题求解代码”组织。附录 A 使用“文件名称、文件说明”两列三线表，逐项对应实际交付的解题程序、输入数据和必要结果文件；附录 B 依次列出 Q1、Q2、Q3 的主要建模代码，有 Q4 时增加 Q4，无 Q4 时删除该节。文件名称表不得替代代码；主要代码缺失、未定位或以截图/非核心辅助代码冒充时立即 `BLOCKED_APPENDIX_CODE/P0`。`appendix_start_pdf_page` 指向附录 A 起始页，`appendix_code_pdf_page` 指向附录 B 首个实际代码页。内部冻结清单、哈希、验证报告、追踪表和图形注册表仍保留在论文外部，不直接复制成附录 A；AI 使用披露仍按当届官方规则单独处理。
- 当届规则允许彩色时，附录代码统一使用 `code_theme=VS-CODE-LIGHT-PLUS`：参考 VS Code 浅色语义高亮，以极浅灰底、等宽、可复制文本对关键字、函数、类、字符串、注释和数值等角色使用Light+ 语义多色字体，并用加粗/斜体补充灰度冗余。全文只用同一代码主题，代码主题与图表 `palette_set` 分开登记，不得逐块换色、随机配色、使用深色编辑器背景或代码截图；官方要求黑白时记录规则例外。
