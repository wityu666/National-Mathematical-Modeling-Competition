...
- 写作骨架只提供内容要求，不提供可照抄句式；图表资产提供六组已验证候选配色，由每队显式选定一组并保持全文一致。
...
- 论文图须在出图前从多组已验证候选中显式选择并登记 `palette_set`，全文只使用该组且同一对象映射一致；各组按统一 CIELAB L* 阶梯与逐组色相旋转设计，线型、标记或直接标签继续作为灰度与色觉缺陷条件下的冗余编码。
