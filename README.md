...
- 当届规则允许彩色时，附录代码统一使用 `code_theme=VS-CODE-LIGHT-MUTED`：参考 VS Code 浅色语义高亮，以白底、等宽、可复制文本对关键字、函数、类、字符串、注释和数值等角色使用克制的多色字体，并用加粗/斜体补充灰度冗余。全文只用同一代码主题，代码主题与图表 `palette_set` 分开登记，不得逐块换色、随机配色、使用深色编辑器背景或代码截图；官方要求黑白时记录规则例外。
