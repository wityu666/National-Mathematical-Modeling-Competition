...
一组面向中国大学生数学建模竞赛（CUMCM）比赛进行中的 Codex skills。技能覆盖赛题拆解、内置方法模式匹配、模型设计、Python/MATLAB 实现、论文成稿与提交前终审，并用 `CONTRIB-*` 亮点账本把可验证的差异化成果贯穿全链路。
...
problem_contract -> pattern_matches -> model_contract
                                      ├─> run_manifest -> evidence_ledger ─┐
                                      └─> contribution_ledger ────────────┴─> paper -> audit_report
```
...
统一证据标识使用 `Q-* / DATA-* / MODEL-* / RUN-* / RID-* / FIG-* / TAB-* / CONTRIB-* / ISSUE-*`。亮点记录使用 `CANDIDATE / PROVEN / DROPPED`；套件产物仍使用 `DRAFT / FROZEN / STALE / BLOCKED / PASS`。
